...
def main (args):
    ...
    a='a'
    while(a!='@'):
        a=input("Introduce un simbolo del alfabeto (@ para terminar):")
        if (a!='@'):
            alfabeto.append(a)
    b=0
    while(b!=-1):
        b=int(input("Introduzca el estado inicial (-1 para terminar):"))
        if(b!=-1):
            c=int(input("Introduzca el estado final:"))
            d=input("Introduzca la transicion:")
            listaT.append (transicion (b, c, d))
        ...
        for j in alfabeto:
            ...
            if (pertenece (U, Estados) == 0):
               # print ("entro")
                ...
                #print (temp2)
            aux = tranSub(Estados[i].lista, temp2, j)
